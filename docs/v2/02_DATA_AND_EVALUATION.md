...
- `source_observed_at`: 원천 API가 명시적으로 제공한 관측 시각. 필드가 없으면 `null`이며 추정해서 채우지 않는다.
- `observed_at`: 외부 계약에서 원본 수량과 함께 제공하는 기준시각. `source_observed_at`이 있으면 그 값, 없으면 `collected_at`을 사용하고 `observed_at_basis="collected_at"`을 함께 반환한다.
...
|---|---|---|---|---|
| F-01 | 직접 API 경로는 `GET /v1/openapi/station`, `api-token` 헤더를 요구함 | 확인 | 무권한 응답과 다수 독립 사용례가 있으나, 우리 승인 키의 200 응답은 아직 별도 게이트 대상 | `fixtures/api/direct/200_01.redacted.json`, redacted headers |
| F-02 | 직접 API의 정류장 필드는 `id,name,x_pos,y_pos,address,parking_count`로 추정됨 | 추정 | 서드파티 실응답 교차검증 기준이며 우리 승인 키의 200 응답은 아직 없음. `name_en/name_cn`도 실제 표본 미확인 | 실제 200 응답 schema report |
| F-03 | 직접 API 표본에는 개별 자전거 ID·고장 상태·총 용량이 없는 것으로 추정됨 | 추정 | 서드파티 표본 기준이며 우리 승인 200 응답 전에는 부재를 확정하지 않음. 고장 자전거가 `parking_count`에 반드시 남는다는 뜻도 아님 | 실제 200 응답의 키 집합 |
| F-04 | `x_pos`는 위도, `y_pos`는 경도로 해석될 가능성이 높음 | 추정 | 서드파티 표본의 대전 좌표 범위로 교차검증했지만 우리 승인 키의 실제 200 응답은 없음 | 실제 200 fixture의 좌표 범위·지도 표본 대조 결과 |
| F-05 | data.go.kr 15109253은 자동승인·개발계정 10,000회/일로 안내됨 | 카탈로그확인 | 실제 키·호출·응답·페이지네이션은 미검증 | `fixtures/api/datago/200_01.redacted.json`, response metadata |
| F-06 | data.go.kr 카탈로그는 거치대 수와 이용 가능 수량을 안내함 | 카탈로그확인 | 필드명·의미·nullability·실제 제공 여부 미확인 | schema report와 표본 분포 |
| F-07 | 직접 API와 data.go.kr가 동일 시점·동일 원천 데이터를 제공함 | 미확인 | 등록 경로와 승인 방식이 다름 | §3 동시 수집 비교 보고서 |
| F-08 | 대여이력 CSV는 자전거번호, 대여·반납 시각·장소 등의 trip event를 제공함 | 카탈로그확인 | 실제 다운로드 파일의 헤더·기간·행수·인코딩은 §5 통과 전 확정하지 않음 | 원본 SHA-256, CSV 품질 보고서 |
| F-09 | 대여이력은 연 1회 갱신되고 최신 카탈로그 파일명은 2026-03-31 기준임 | 카탈로그확인 | 파일의 실제 최소·최대 이벤트 시각이 1년 범위라는 의미는 아님 | min/max 시각, 월별 행수 |
| F-10 | trip events만으로 과거 절대 `parking_count`를 재구성할 수 없음 | 확인 | 초기 재고, 재배치·정비 이동, 누락 이벤트가 없기 때문 | 해당 없음(데이터 구조상 한계) |
| F-11 | 과거 station snapshot을 충분한 기간 확보할 수 있음 | 미확인 | 서드파티 저장소가 최신 JSON만 보존하는지 commit history까지 보존하는지 검증되지 않음 | snapshot 기간·간격·결측 보고서 |
| F-12 | `parking_count`에 잡힌 자전거가 실제로 대여 불가한 사례가 반복됨 | 추정 | 민원·현장 사례는 있으나 API값·현장 상태를 같은 시각에 결합한 표본이 없음 | 블라인드 현장 점검 라벨 |
| F-13 | 기상청 단기예보 조회서비스가 초단기실황·초단기예보·단기예보를 제공함 | 카탈로그확인 | 실제 승인 키 응답, 대전 격자 매핑, 발행·유효시각·revision 의미는 미검증 | `fixtures/context/weather_forecast/200_01.redacted.json`, grid/version report |
| F-14 | 한국천문연구원 특일 정보 API가 공휴일 calendar 후보 원천으로 존재함 | 카탈로그확인 | 실제 응답·개정 공표시각·과거 revision 보존 가능 여부는 미검증 | `fixtures/context/calendar/200_01.redacted.json`, revision report |
| F-15 | 에어코리아 대기오염정보 API가 대기질 후보 원천으로 존재함 | 카탈로그확인 | Core 필수 입력이 아니며 실제 응답·측정소 매핑·증분 성능은 미검증 | `fixtures/context/air_quality/200_01.redacted.json`, optional ablation report |
| F-16 | 대전 행사 일정의 발행·수정·취소 이력을 point-in-time으로 재현할 단일 원천이 있음 | 미확인 | 현재 공식 원천·과거 revision·장소 좌표·예상 규모의 이용 가능성을 확정하지 못함 | source decision record, versioned event fixtures |

### 2.3 근거 링크와 조사 이력

- [타슈 OpenAPI 공식 안내](https://www.tashu.or.kr/noticeDetail.do?seq=27)
...
- [기상청 단기예보 조회서비스](https://www.data.go.kr/data/15139470/openapi.do)
...
1. 직접 API와 data.go.kr 키는 정식 발급 경로로만 얻고 저장소에 커밋하지 않는다.
2. 각 경로를 같은 네트워크에서 **15분 동안 5분 간격으로 3회** 호출한다. 경로 간 호출 시각 차이는 60초 이내로 기록한다.
3. 본문 원본, HTTP 상태, `Content-Type`, 수집시각, 응답 바이트 수, 페이지 관련 메타데이터를 보존한다. 키·쿠키·개인정보는 fixture 생성 전에 제거한다.
4. raw 응답과 정규화 결과를 분리한다. 벌크 응답 전체를 정류장 행마다 중복 저장하지 않는다.
5. 200 외에도 무권한, 잘못된 파라미터, rate limit, timeout fixture를 가능한 범위에서 확보한다. 고의로 한도를 소진하지 않는다.

권장 artifact 구조:

```text
fixtures/api/
  direct/200_01.redacted.json
  datago/200_01.redacted.json
...
```json
{
  "data_source": "live_direct|live_datago|demo",
...
- 원천 `parking_count`를 `bikes_available`로 이름만 정규화하며 차감·보정하지 않는다. 값은 정수이며 `>= 0`이어야 한다. 원천이 문자열이면 어댑터가 명시적으로 변환하고 원본 타입을 보고서에 남긴다.
...
- `capacity`가 존재하더라도 `capacity >= bikes_available`와 시간상 안정성이 확인되기 전에는 `capacity_source='datago_api'`로 승격하지 않는다.

### 3.3 두 경로 비교표

각 200 fixture 쌍마다 다음을 계산한다.
...
두 경로를 “동일 데이터의 대체 경로”로 취급하려면 세 fixture 쌍 모두에서 다음을 만족해야 한다.

- ID Jaccard `>= 0.99`
- 좌표 허용오차 `<= 0.0001°` 내 일치율 `>= 0.99`
- 호출 시각 차이 60초 이하인 쌍에서 `parking_count` exact match율 `>= 0.95` 또는 불일치가 갱신시각 차이로 설명됨
...
- direct 200 최소 3개, data.go 200 최소 3개
...
- 합성 context가 하나라도 들어가면 최종 API `data_source=demo`가 되는 contract test
...
- 기준 snapshot `t`와 목표 snapshot `t+15분 ±2분 30초`가 모두 있어야 표본으로 채택한다.
...
1. **B0 현재값 유지**: `bikes_available(t)>=1`이면 `p_available=1-ε`, 아니면 `p_available=ε`, `ε=0.001`.
...
3. **B2 재고·시간 logistic**: §7.3의 재고 이력과 시간 feature만 사용하는 정규화 logistic regression. station 효과 사용 여부, regularization과 class weight는 calibration 이전에 고정한다.
4. **B3 투명한 맥락 logistic**: B2에 같은 point-in-time 날씨·휴일·행사·주변 집계·정적 공간 feature를 더한 정규화 logistic regression. 변환·bin·상호작용은 사전등록하고 계수 방향·결측 경로를 감사 가능하게 남긴다. 복잡한 M1이 아니라 단순한 맥락 결합만으로 얻는 이득을 분리한다. 자체 contextual 게이트를 통과하면 가장 단순한 `contextual_ml` 후보가 될 수 있다.
5. **M1 contextual tabular 후보**: 재고·시간에 날씨 예보, 휴일, 행사, 주변 정류장, 정적 공간 특성을 더한 CatBoost 또는 LightGBM. 둘을 final test에서 겨뤄 좋은 쪽을 고르지 않으며 algorithm·탐색공간·seed를 사전등록하고 calibration 구간에서 하나만 선택한다.
6. **M2 고급 공간 모델**: temporal graph/attention 등 고급 공간 구조는 M1의 맥락 uplift가 미래 holdout에서 승인된 뒤에만 연구한다. M2 착수·복잡성 자체를 성능 근거로 발표하지 않는다.

B3와 M1은 B0/B1뿐 아니라 B2와 **동시에 비교**한다. 둘 다 B2를 이기지 못하면 맥락 추가 가치가 없으므로 “날씨·행사·공간 맥락을 고려해 더 정확한 AI”라고 주장하지 않고 B2 또는 §8 fallback으로 강등한다. B3가 B2를 이겼지만 M1이 B3를 이기지 못하면 더 복잡한 M1을 배포하지 않고 B3를 `contextual_ml` 후보로 쓴다. 후보 선택 규칙은 final test 전에 고정하며 모델 종류가 아니라 point-in-time 데이터와 미래 holdout의 추가 성능이 AI 기능의 근거다.

### 7.5 사전등록 ablation과 맥락 slice

비선형 모델 자체의 효과와 맥락 데이터의 효과를 분리하기 위해 같은 M1 algorithm·hyperparameter budget으로 다음 중첩 실험을 사전등록한다.
...
M1-IWH    = M1-IW + 휴일
M1-IWHE   = M1-IWH + 행사
M1-FULL   = M1-IWHE + 주변 정류장 + 정적 공간
```

- `B2 vs B3`는 투명한 맥락의 추가 가치, `B3 vs M1-FULL`은 비선형 contextual 모델의 추가 가치, `B2 vs M1-I`는 맥락 없는 model family 효과, `M1-I vs M1-FULL`은 같은 family에서의 전체 맥락 추가 가치를 본다.
- 순서 의존성을 확인하기 위해 `M1-FULL`에서 날씨·휴일·행사·주변/공간 그룹을 하나씩 제거한 leave-one-group-out 결과도 함께 보고한다.
- full 결과가 나쁜 그룹만 사후 제거해 같은 holdout 점수를 “최종 성능”으로 다시 쓰지 않는다. feature 변경은 새 모델 버전과 새 미래 holdout을 요구한다.
- 각 비교는 Brier·log loss·false-go와 station 또는 event 단위 bootstrap 95% CI를 함께 보고한다. 표본 수와 CI 없이 작은 차이를 uplift로 부르지 않는다.
...
| 재고·시간 | 현재 0·1·2·3대 이상, 출퇴근/비출퇴근, 결측률 구간 |
...
| 범주 | 지표 | 사용법 |
|---|---|---|
| 주 지표 | **Brier score** | B0/B1/B2/B3 대비 상대 개선율과 station bootstrap 95% CI 보고 |
...
| 예비 비교 | 최소 42일 | B0/B1/B2/B3/M1 예비 비교, calibration·slice 결측 발견 | 일반화된 contextual uplift 또는 공개 성능 보장 |
...
현재 문서의 초기 **권고 기준**은 모든 시간 test fold에서 B0 대비 Brier 개선, B1 대비 비열화 없음, 사전 선택된 contextual 후보(B3 또는 M1)의 B2 대비 uplift에 대한 pooled bootstrap 95% CI 하한 `> 0`, ECE `<= 0.10`, B2와 동일 GO coverage에서 false-go 비열화 없음이다. M1을 선택하려면 B3 대비 추가 가치도 별도로 보고한다. 이 수치를 그대로 운영 승인으로 간주하지 않는다. `evaluation_gate`는 최소한 `inventory_temporal`과 `contextual_ml` basis별로 따로 채택·서명한다. 승인된 contextual 후보가 없으면 별도 게이트를 통과한 B2 계열 모델을 `decision_signal.mode="stockout_risk"`, `prediction_basis="inventory_temporal"`로 사용한다.
...
1. 최신 현재 snapshot이 있고 사전 선택된 contextual 후보(B3 또는 M1)의 게이트를 통과하면 `stockout_risk/contextual_ml`을 제공한다.
...
- `demand_pressure`는 B0/B1/B2/B3/M1과 별개의 fallback baseline이다. 날씨·행사·공간 feature를 섞어 “AI 예측”으로 부르거나 `stockout_risk` 성능과 한 표에서 같은 타깃처럼 비교하지 않는다.
...
| M1 algorithm·hyperparameter budget | 사전등록한 CatBoost 또는 LightGBM 범위 | train/calibration만, final holdout 비교 선택 금지 |
...
- [ ] direct API 실제 200 fixture와 검증 보고서가 있다.
- [ ] data.go 실제 200 fixture와 검증 보고서가 있다.
- [ ] 두 경로를 동일/별개로 판정했다.
- [ ] CSV 원본 해시·기간·유효/격리 행수·station 매핑률이 있다.
...
- [ ] B0/B1/B2/B3/M1, 중첩·leave-one-group-out ablation, 시간·정류장·행사 holdout과 Brier/log loss/calibration/동일 coverage false-go가 자동 산출된다.
