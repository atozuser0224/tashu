...
5. **오프라인 데모는 운영 모드와 같은 계약을 쓴다.** fixture도 동일 OpenAPI 응답을 반환하며, 최신성은 `fresh/stale/unavailable`, 전체 실행 모드는 `response_mode=demo`로 숨김없이 표시한다. `data_source`는 공식 재고 원천만 뜻한다.
...
```mermaid
flowchart LR
    SRC[타슈 직접·전국 통합 재고 API 또는 demo fixture] --> POLL[독립 재고 poller]
...
3. 서버는 공식 재고의 `data_source`·`data_freshness`를 먼저 판정한 뒤, 맥락 특징을 point-in-time으로 조립한다. 검증된 `contextual_ml`을 우선하고 불완전한 맥락을 0으로 채우지 않은 채 `inventory_temporal`, `current_stock` 순으로 폴백한다. policy가 등급·행동 코드를 결정하며, 대안은 `data_freshness=fresh`인 정류장만 최대 3개 반환하고 각각 공식 재고·출처·기준시각을 포함한다. list/forecast parent와 모든 item은 같은 `response_mode`를 사용한다.
...
| `source` | `tashu_direct_inventory`, `national_integrated_inventory`, `fixture` |
| `mode` | `live`, `demo` |
...
| 외부 계약 `data_source` | `ingestion_run.source` | `ingestion_run.mode` |
|---|---|---|
| `direct_tashu` | `tashu_direct_inventory` | `live` |
| `national_integrated` | `national_integrated_inventory` | `live` |
| `demo` | `fixture` | `demo` |

타슈 직접 API와 전국 통합 API는 한 요청 안에서 섞는 fallback chain이 아니라 별도의 scheduler·run·raw payload로 이중 수집한다. 두 원천 행을 합쳐 하나의 snapshot을 만들지 않으며, 서빙은 품질 게이트를 통과한 versioned 선택 정책이 snapshot 하나를 고른 뒤 그 snapshot의 `data_source`를 그대로 반환한다. 원천 간 정류장 ID 대응표도 version을 갖고, 매핑 불명·충돌 행은 자동 병합하지 않는다.

공공데이터포털 데이터셋 ID `15109253`은 live 재고 source가 아니다. 정류장명·주소·좌표 등 필드를 실제 응답과 대조하고 이용약관·갱신주기·ID 안정성을 검증한 뒤에만 사용할 수 있는 **static metadata candidate**다. 이 데이터셋의 값으로 `bikes_available`, `observed_at` 또는 재고 `data_source`를 만들지 않는다.

live adapter를 켜기 전에 원천별 호출 한도, 일·분당 요청 예산, 페이지 크기 상한, 전체 페이지 수 산정, 마지막 페이지 판정, 중복·누락 검사를 기록한 rate/page gate를 통과한다. 모든 페이지를 받지 못한 cycle은 `partial`로 남기고 완전한 정류장 집합이나 삭제 신호로 사용하지 않는다. `4xx`를 일괄 재시도하지 않으며 인증 오류는 즉시 운영 경보, `429`는 `Retry-After`+jitter, 타임아웃·일부 `5xx`만 제한 횟수 재시도한다. TLS 검증은 절대 끄지 않는다.
...
| `observed_at` | 외부 계약에서 사용하는 관측 기준시각. v2.2에서는 `collected_at`과 동일 |
| `collected_at` | 우리 수집기가 해당 응답 또는 fixture를 수집 완료한 시각. run의 `received_at`에서 전달 |
| `observed_at_basis` | v2.2 성공 관측에서는 `collected_at`. 원천 관측시각이 아님을 명시 |
...
- 이 v2.2의 두 live API는 신뢰할 수 있는 원천 관측시각을 제공하지 않는다. 성공 관측은 `observed_at=collected_at`, `observed_at_basis=collected_at`이어야 하며 수집시각을 원천 갱신시각으로 표현하지 않는다. 성공 관측이 없는 unavailable 상태에서는 `observed_at`, `collected_at`, basis가 모두 null일 수 있다.
- 원시 snapshot은 원천 정책과 rate/page gate가 허용하면 1분 수집을 선호하고 성공 응답을 그대로 보존한다. 모델 입력은 별도 5분 grid로 만들되 각 grid 시각 이하의 최신 관측만 as-of 선택한다. 미래 방향 nearest join, 선형보간, 원천 간 혼합을 금지하고 선택된 snapshot의 `data_source`, `collected_at`, age를 feature provenance에 남긴다.
...
같은 행사·날씨 예보가 수정돼도 기존 행을 덮어쓰지 않고 새 version으로 append한다. 학습 시점의 발행 내용을 재현할 수 없는 최신본-only 데이터는 contextual feature로 승격하지 않는다. 합성 맥락이 하나라도 전체 feature provenance에 들어가면 `uses_synthetic_context=true`, 응답 전체를 `response_mode=demo`로 강등하고 실제 이동 추천·운영 성능 집계에서 제외한다. 공식 재고가 live면 `data_source=direct_tashu|national_integrated`는 그대로 유지한다.
...
각 prediction log는 최소 `prediction_id`, `station_id`, `prediction_at`, `target_at`, `horizon_minutes`, 공식 `station_snapshot` ID, 사용한 `context_snapshot_id[]`, `feature_contract_version`, `prediction_basis`, `context_status`, 누락 kind, `uses_synthetic_context`, `response_mode`, `model_version`, `calibration_version`, `threshold_version`, 원시 score와 보정 확률, 최종 mode/grade를 기록한다. 사용자 위치와 외부 credential은 기록하지 않는다. 공개 응답의 `context_evidence`는 이 provenance에서 최대 2개의 구조화 사실만 고르며, 선택 evidence가 모두 실데이터여도 전체 provenance에 합성이 있으면 flag를 false로 낮추지 않는다. `direction`은 신호에 대한 연관 방향이지 인과 설명이 아니다.
...
모든 폴백은 동일 요청 안에서 결정적으로 수행하며 사유 코드를 내부 로그에 남긴다. `context_status`와 `response_mode`는 공식 재고 최신성·원천의 대체값이 아니며 `data_source`·`data_freshness`를 변경하지 않는다. 공식 재고 fixture로 `data_source=demo`이면 response mode도 demo이고, 합성 맥락을 사용한 live 재고 응답은 data source를 유지한 채 response mode만 demo다.
...
`response_mode=live|demo`는 데이터 의미가 아니라 응답 전체의 실행·표시 모드다. StationList와 station items, ForecastResponse와 alternatives는 parent mode를 공유한다. `data_source=demo` 또는 `uses_synthetic_context=true`이면 response mode는 demo지만, 합성 맥락만 사용한 live 재고 응답의 data source는 원래 공식 원천을 유지한다.

### `decision_signal` mode와 grade 방향

- `mode=stockout_risk`: 최신 snapshot과 승인된 예측 basis를 사용할 수 있을 때 산출한다. `horizon_minutes=15`, `stockout_probability=0..1`, `stockout_risk_grade=low/medium/high`이며 **high는 대여 가능성이 높다는 뜻이 아니라 소진 위험이 높다는 뜻**이다. 확률은 정확히 t+15 공식 재고가 1대 미만일 확률이고 `demand_pressure_grade`는 null이다.
- `mode=demand_pressure`: snapshot 없이 과거 대여 이력만으로 15분 수요 압력을 표현한다. `horizon_minutes=15`, `demand_pressure_grade=low/medium/high`이고 `stockout_probability`, `stockout_risk_grade`는 null이다. 이는 현재 재고 소진 확률이 아니다.
- `mode=current_stock`: 최신 공식 재고만 안전하게 보여줄 수 있다. `horizon_minutes`, `stockout_probability`와 두 grade는 null이다.
- `mode=unavailable`: 판단 자료가 부족하다. `horizon_minutes`, `stockout_probability`와 두 grade는 null이다.
...
graded mode는 non-null `prediction_basis`와 `feature_contract_version`을 함께 반환한다. `prediction_basis=contextual_ml`이면 `context_status=complete`, non-null `calibration_version`, fresh `weather|calendar|event|neighbor|temporal` evidence 1~2개를 요구하고 inventory evidence를 금지한다. `weather|calendar|event`는 non-null `issued_at`, `neighbor|temporal`은 null issued_at과 기준시각 `valid_at`을 사용한다. `inventory_temporal`은 fresh 공식 재고 lag/rolling+시간 기반의 검증된 폴백이며 non-null calibration version과 fresh inventory evidence 1~2개만 허용한다. `historical_demand`는 no-snapshot trip fallback으로 `context_status=unavailable`, `calibration_version=null`, `context_evidence=[]`다. `current_stock|unavailable`은 세 모델 provenance를 null로 반환하고 evidence를 빈 배열로 둔다. 모든 mode는 `uses_synthetic_context`를 반환하며 stockout 외에는 false다.

최신성과 결정 신호도 독립 축이지만 허용 조합은 제한한다. `fresh`는 `stockout_risk|current_stock`, `stale|unavailable`은 `demand_pressure|unavailable`만 가능하다. `StationState`와 원자 `ForecastResponse`가 `fresh`이면 `inventory`도 반드시 non-null이다. compatibility schema가 `stale+stockout_risk`, `fresh+inventory:null` 같은 모순 응답을 거부한다. 최대 3개의 대안 후보는 모두 `fresh`이고 `stockout_risk|current_stock`만 사용하며, 주소가 없을 수 있어도 공식 재고·data source·response mode·관측 기준시각·예상 도보시간을 반드시 포함한다.
...
- demo 여부는 최신성·공식 재고 원천과 독립인 `response_mode=demo`로 표시한다. 공식 재고 fixture는 `data_source=demo`도 함께 가지며, live 공식 재고에 합성 맥락만 사용한 응답은 원래의 `data_source=direct_tashu|national_integrated`를 유지하고 `response_mode=demo`만 사용한다. fixture도 고정 clock 기준으로 `fresh/stale/unavailable` 중 하나를 가진다.
...
최신성 age는 API 응답의 `observed_at`으로 계산한다. v2.2에서는 성공 관측이 `observed_at=collected_at`, `observed_at_basis=collected_at`이므로 이 age는 **우리의 마지막 성공 수집 age**일 뿐 원천 내부 데이터 age나 갱신 SLA를 증명하지 않는다.
...
- 입력: 서버가 계산·검증한 `fact_key`, 값, 단위, 이미 결정된 `decision_signal.mode`, `horizon_minutes`, 승인된 `stockout_probability`와 해당 grade, `prediction_basis`, `context_status`, `uses_synthetic_context`, 최대 2개의 `context_evidence`, `inventory_signal`, `action_code`.
...
- 수집: 재고·context adapter별 run 성공/부분/실패율, source별 latency, 요청 예산·429, 기대/완료 page 수, item/invalid count, payload hash 반복시간, 마지막 성공 수집 age와 schema drift.
- API: endpoint별 요청수·p50/p95/p99·4xx/5xx, idempotency replay/conflict, 429, capability 거부.
- 모델/제품: 공식 data freshness와 별도로 `response_mode`, `uses_synthetic_context`, `context_status`, `prediction_basis`, feature kind별 누락·age, contextual→temporal→current-stock→historical-demand→unavailable 폴백 사유, model/calibration version별 보정 오차·drift, `decision_signal.mode`, unavailable 비율을 기록한다. community report 생성/취소량은 모델 feature나 고장 정확도 지표로 오인하지 않는다.
- LLM: 호출률, template fallback률, 지연, schema 실패, token/비용. LLM 문장은 품질 표본검사 대상이지 상태 정답이 아니다.

로그·trace는 공통 `request_id`, 재고/context `run_id`, `station_id`, `response_mode`, `horizon_minutes`, `feature_contract_version`, `model_version`, `calibration_version`, `policy_version`으로 연결한다. readiness는 DB와 adapter별 마지막 성공·feature builder·폴백 predictor 상태를 확인하되, LLM 장애만으로 실패시키지 않는다. 알림에는 upstream token, credential URL, 사용자 위치·검색어·본문을 포함하지 않는다.

### 오프라인 데모

- `DEMO_MODE=true`면 네트워크 egress를 차단하고 버전·해시가 있는 fixture manifest를 읽는다.
- clock을 고정하거나 fixture 기준시각에 상대 이동시켜 결과를 재현 가능하게 한다.
- 응답은 항상 `response_mode=demo`를 반환하고 화면 워터마크를 강제한다. 공식 재고도 fixture이면 `data_source=demo`, live 재고를 재사용하면 원래 live data source를 유지한다. freshness는 fixture clock 기준의 `fresh/stale/unavailable`을 사용한다.
- 합성 context fixture는 각 snapshot에 `synthetic=true`를 기록하고 전체 feature provenance에서 `uses_synthetic_context=true`를 산출한다. 공개 evidence에 선택되지 않았더라도 flag와 demo response mode는 유지하며, evidence의 source·시각은 fixture manifest로 재현 가능해야 한다.
...
2. `ingestion_run`/`station_snapshot` migration과 타슈 직접·전국 통합 독립 poller, fixture poller.
...
- [ ] `direct_tashu`와 `national_integrated`가 독립 run·raw payload로 수집되고, versioned source 선택 정책과 ID 대응표 없이 두 원천 행을 혼합하지 않는다. `15109253`은 static metadata candidate로만 격리된다.
- [ ] live adapter별 rate/page gate가 호출 예산·페이지 완주·중복·누락을 검증하고, 페이지 일부 실패를 `partial`로 기록하며 삭제나 완전 snapshot으로 오인하지 않는다.
- [ ] 허용 시 1분 raw 관측을 보존하고 5분 학습 grid가 과거 방향 as-of만 사용한다. 미래 관측·선형보간·원천 간 혼합 반례는 실패한다.
- [ ] 다섯 축이 OpenAPI 응답에서 독립 객체로 존재하며 단일 badge, community/LLM이 official maintenance를 변경할 경로가 없다.
- [ ] `decision_signal.mode`별 oneOf 계약이 `horizon_minutes`, `stockout_probability`와 반대 grade의 nullability를 강제하고, 소진 위험 high를 “대여 가능성 높음”으로 렌더링하지 않는다.
...
- [ ] `prediction_basis=contextual_ml`은 complete context와 fresh `weather|calendar|event|neighbor|temporal` evidence에서만 통과하고 inventory evidence를 거부한다. weather/calendar/event의 null issued_at, neighbor/temporal의 non-null issued_at도 거부한다. inventory_temporal은 fresh inventory evidence에서만 통과한다.
...
- [ ] 모든 decision mode가 `uses_synthetic_context`를 가지며 stockout 외에는 false다. 전체 provenance 또는 선택 evidence에 합성이 하나라도 있으면 flag=true와 `response_mode=demo`이고, live 공식 재고의 `data_source`는 바꾸지 않는다.
- [ ] `data_source=demo → response_mode=demo`, StationList parent↔station item, Forecast parent↔alternative item의 response mode 일치를 JSON Schema가 강제한다.
- [ ] fresh/stale/unavailable과 decision mode의 금지 조합, 대안의 fresh+허용 mode 조건을 JSON Schema와 응답 fixture 모두에서 거부한다.
- [ ] ForecastResponse는 nullable inventory를 원자적으로 포함하며 fresh/current_stock이면 non-null이다. alternatives는 최대 3개, 모두 fresh이고 공식 재고·data source·response mode·기준시각을 가진다.
...
- [ ] freshness 단위 테스트가 `fresh/stale/unavailable` 경계를 고정 clock으로 통과하고 demo response mode와 공식 data source가 독립적으로 노출된다.
- [ ] 성공한 live/fixture 관측은 `observed_at=collected_at`, `observed_at_basis=collected_at`이고, 원천 갱신시각으로 렌더링되지 않는다. unavailable에서는 세 필드가 모두 null일 수 있다.
- [ ] OpenAPI와 enum artifact가 모두 v2.2.0이며 `DataSource=direct_tashu|national_integrated|demo`만 허용하고 구 v2.1 source 값과 선언되지 않은 `$ref`를 계약 테스트가 거부한다.
