...
except ImportError:  # Support `python snapshot_engine.py ...` from Hosu/.
    from model import A3TGCN
...
def compute_snapshot(date=None, round_id=None, mode='demo', demo_mode=True):
    ...
    row=tl.iloc[target]
    hour, minute = (int(part) for part in ROUND_TIME[row['round_id']].split(':'))
    day_offset = 1 if row['round_id'] == 'D' else 0
    ref_time = (
        pd.Timestamp(row['service_date'])
    ...
    return {"meta":{"date":str(row['service_date'].date()),"round_id":row['round_id'],
                    ...
                    "service_day_note":"D는 서비스일 다음 날 03:00",
                    "inventory_source":"synthetic_daily_baseline_8_plus_observed_flow",
