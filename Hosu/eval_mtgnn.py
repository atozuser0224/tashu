...
model=MTGNN(f_node=Xn.shape[-1],f_global=Xg.shape[-1],n_nodes=Xn.shape[2],hidden=48,emb_dim=16)
model.load_state_dict(torch.load(f'{OUT_DIR}/mtgnn.pt',map_location='cpu')['state']); model.eval()
