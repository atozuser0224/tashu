...
- **방향(비대칭) 인접행렬 + 양방향 GCN**: 출근/퇴근의 이동 방향 경향(어디서 어디로)을
  살리기 위해 대칭화를 제거하고 A(나감)/Aᵀ(들어옴) 양방향 전파. 균형정확도 +2.2%p 개선.
...
# 7) STGNN 분류 학습 (부족/정상/과잉, 양방향 GCN) -> a3tgcn_dir.pt
python train_cls_directed.py

# 7-1) (선택) 혼동행렬로 치명적 오류율 확인
python eval_confusion_directed.py
...
모델 성능 (test, 2026년 3월 홀드아웃): 균형정확도 0.465 (무작위 0.333),
치명적 오류(부족<->과잉 혼동) 13.9%. 확률 기반 보조지표로 설계, 최종 판단은 관제팀.